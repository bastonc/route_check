...
from utlity.check_route_thread import CheckRouteThread, CsvWorker
...
class GUI_worker(CheckRouteWnd):
    ...
    def start_analyze_route(self):
        ...
        self.scv_worker_thread.articles_list.connect(self.fill_csv_list)
        ...
        #     # stop_position = None if (start_position + count_articles_for_thread) > count_articles_for_thread else start_position + count_articles_for_thread
        #     self.thread_analyze.append(CheckRouteThread(articles_list=self.all_articles_list[
        #                                                               start_position: None if (start_position + count_articles_for_thread) > count_articles_for_thread else start_position + count_articles_for_thread],
        ...
        self.worker_thread = CheckRouteThread(csv_name=self.csv_file_name,
                                              exclude_route=self.exclude_route,
                                              exclude_url_list=self.exclude_url_list,
                                              skin=self.skin_values[self.skin_combo.currentText()]["url_pattern"],
                                              class_name=self.skin_values[self.skin_combo.currentText()]["bc_class_link"],
                                              output_csv_name="result_gui_4.csv",
                                              regex_pattern=self.skin_values[self.skin_combo.currentText()]["regex_pattern"])
        #self.worker_class.moveToThread(self.work_thread)
        self.worker_thread.finish_processing.connect(self.finish_processing)
        self.worker_thread.console.connect(self.output_to_console)
        #self.work_thread.started.connect(self.worker_class.start_analyze)
